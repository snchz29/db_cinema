...
class FilmsActors(Toplevel):
    ...
    def __init__(self, fid, **kw):
        ...
        for i in range(len(self.roles)):
            ...
            self.lines[i]['button_role'] = Button(self.lines[i]['frame'], width=5, text="Удалить")
    ...
    def push_data(self):
        for i in range(len(self.roles)):
            self.lines[i]["entry_name"].insert(0, self.roles[i][1])
            self.lines[i]["entry_surname"].insert(0, self.roles[i][2])
            self.lines[i]["entry_role"].insert(0, self.roles[i][5])
